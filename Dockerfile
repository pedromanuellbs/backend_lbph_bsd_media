...
# Install dependency sistem (wajib untuk build ML, opencv, face_recognition, dsb)
...
# Instalasi PyTorch dan Torchvision CPU langsung dari index-url resmi
RUN pip install --no-cache-dir torch==2.2.2+cpu torchvision==0.17.2+cpu --index-url https://download.pytorch.org/whl/cpu

# Install semua dependency lain dari requirements.txt (kecuali torch/torchvision)
RUN pip install --no-cache-dir -r requirements.txt

# Salin semua file project ke container
COPY . .

# Download model dari storage (jika diperlukan)
...
# Jalankan aplikasi dengan gunicorn saat container start
