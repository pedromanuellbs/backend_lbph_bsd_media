...
def get_all_gdrive_folder_ids():
    ...
    db = firestore.client() #
    # Change: Store dictionaries of {'firestore_doc_id': ..., 'drive_folder_id': ...}
    folder_data = [] 
    ...
    try:
        ...
        for doc in sessions: #
            ...
            if 'folders/' in drive_link: #
                ...
                # Store both the Firestore doc ID and the Drive folder ID
                folder_data.append({'firestore_doc_id': doc.id, 'drive_folder_id': folder_id}) 
                print(f"     - ID Folder berhasil diekstrak: {folder_id}, Firestore Doc ID: {doc.id}") #
            else:
    ...
    print(f"  > Fungsi selesai. Daftar folder_data yang akan dikembalikan: {folder_data}") #
    print("--- Selesai get_all_gdrive_folder_ids ---\n") #
    return folder_data # Return the list of dictionaries
...
# Change: Add firestore_doc_id parameter
def find_matching_photos(user_face_path, drive_folder_id, firestore_doc_id, threshold=108): 
    user_img = cv2.imread(user_face_path) #
    if user_img is None: #
        print(f"Error: Gagal membaca file wajah user di {user_face_path}") #
        return [] #

    photos_in_folder = list_photo_links(drive_folder_id) # Use drive_folder_id to list photos
    matched_in_folder = [] #

    print(f"Memeriksa {len(photos_in_folder)} foto di folder {drive_folder_id}...") #
        ...
        try:
            ...
            if is_face_match(user_img, target_img, threshold): #
                ...
                matched_in_folder.append({
                    ...
                    'sessionId': firestore_doc_id, # <--- IMPORTANT: Use Firestore doc ID here
                    'webContentLink': photo.get('webContentLink') # Add this for better quality images
                })
            else:
...
# Change: Update find_all_matching_photos to use the new folder_data structure
def find_all_matching_photos(user_face_path, all_folder_data, threshold=108): 
    all_matches = [] #
    for folder_item in all_folder_data: #
        firestore_doc_id = folder_item['firestore_doc_id'] #
        drive_folder_id = folder_item['drive_folder_id'] #
        # Pass both IDs to find_matching_photos
        matches = find_matching_photos(user_face_path, drive_folder_id, firestore_doc_id, threshold) #
